...
SwChannel* SwChannel_init(SwChannel *self)
{
    self->long_integ = 0;
    return self;
}
...
int main()
{
    ...
    static SwChannel channels[CHAN_NUM];
    for (int chan_id = 0; chan_id < CHAN_NUM; ++chan_id)
    {
        SwChannel *chann = channels + chan_id;
    ...
    while (++isamp)
    {
        ...
        if (isamp >= next_meas_idx)
        {
            ...
            for (int chan_id = 0; chan_id < CHAN_NUM; ++chan_id)
            {
                ...
                TrackLoop *track = &chann->track;
                int64_t seqFractQ32;
                int64_t carrCounterQ10;
                TrackLoop_meas(track, &seqFractQ32, &carrCounterQ10);

                //if (chan_id == 0)
                //    printf("chip = %.3f \n", (seqFractQ32 * 1023) / (double) (1LL << 32));

                MeasLogicGps_next_range(measLogic, chan_id, seqFractQ32, -carrCounterQ10);
            }
