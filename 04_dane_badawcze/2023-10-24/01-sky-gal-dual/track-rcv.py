...
while True:
    ...
    if False and sys.stdin in select.select([sys.stdin], [], [], 0)[0]:
        ...
        ser.write(struct.pack("<bhhhh", 1,  2,  round(121.6), round(33.0),  round( 2188 * 0.1902937 * 8)))
        ser.write(struct.pack("<bhhhh", 1,  3,  round(  6.8), round( 3.6),  round(  162 * 0.1902937 * 8)))
        ser.write(struct.pack("<bhhhh", 1,  6,  round( 71.9), round(33.3),  round(   46 * 0.1902937 * 8)))
        ser.write(struct.pack("<bhhhh", 1, 12,  round( 34.1), round(83.1),  round( -318 * 0.1902937 * 8)))
        ser.write(struct.pack("<bhhhh", 1, 14,  round(286.4), round(33.3),  round(  -96 * 0.1902937 * 8)))
        ser.write(struct.pack("<bhhhh", 1, 17,  round( 47.1), round( 4.3),  round(-3459 * 0.1902937 * 8)))
        ser.write(struct.pack("<bhhhh", 1, 20,  round( 16.5), round( 2.2),  round(  326 * 0.1902937 * 8)))
        ser.write(struct.pack("<bhhhh", 1, 24,  round(162.3), round(49.9),  round(-2525 * 0.1902937 * 8)))
        ser.write(struct.pack("<bhhhh", 1, 25,  round(278.5), round(49.8),  round( 2163 * 0.1902937 * 8)))
        ser.write(struct.pack("<bhhhh", 1, 29,  round(216.4), round(16.9),  round( 3684 * 0.1902937 * 8)))
        ser.write(struct.pack("<bhhhh", 1, 31,  round(313.9), round( 4.6),  round( 3527 * 0.1902937 * 8)))
        ser.write(struct.pack("<bhhhh", 1, 32,  round(332.3), round( 1.7),  round(-1416 * 0.1902937 * 8)))
    ...
    if id == 0 :
        ...
        qp = rd[3] * 32
        if chan == SAVE_CHAN :
            csv.write("{}; {}; {}; {}\n".format(idx * MUL_CSV_TIME, chan, ip, qp))
            idx += 1
    ...
    if id == 6:
        ...
        if LOG_RCV:
            msg = "-- obs: [ID {}; Chan {}; Band {}; Prn {}; Rx Tow {}; Carr {}; Range {};]\n".format(id, chan, band, prn, rxtow/2**32, carr/2**10, prange/2**10)
            print(msg, end = '')
            log.write(msg)
